...
and  (year(hd.ngay_lam_hop_dong)  = (2021)) and  (month(hd.ngay_lam_hop_dong)   in (01,02,03))) where hd.ma_hop_dong is null ;
...
join loai_dich_vu ldv on ldv.ma_loai_dich_vu = dv.ma_loai_dich_vu 
left join hop_dong hd on (hd.ma_dich_vu = dv.ma_dich_vu
and  year(hd.ngay_lam_hop_dong)  = (2021)) where hd.ma_hop_dong is null 
)
...
select ho_ten from (select ho_ten from khach_hang group by ho_ten) as kh;

-- 9 Thực hiện thống kê doanh thu theo tháng, nghĩa là tương ứng với mỗi tháng trong năm 2021 thì sẽ có bao nhiêu khách hàng thực hiện đặt phòng.
select month(hd.ngay_lam_hop_dong) , count(month(hd.ngay_lam_hop_dong)) as thang  from khach_hang kh 
join hop_dong hd on hd.ma_khach_hang = kh.ma_khach_hang  and year(hd.ngay_lam_hop_dong) = 2021 group by month(hd.ngay_lam_hop_dong) ;

-- 10 Hiển thị thông tin tương ứng với từng hợp đồng thì đã sử dụng bao nhiêu dịch vụ đi kèm.
-- Kết quả hiển thị bao gồm ma_hop_dong, ngay_lam_hop_dong, ngay_ket_thuc, tien_dat_coc, so_luong_dich_vu_di_kem (được tính dựa trên việc sum so_luong ở dich_vu_di_kem).
select hd.ma_hop_dong , hd.ngay_lam_hop_dong , hd.ngay_ket_thuc , hd.tien_dat_coc , coalesce(sum(hdct.so_luong),0) from hop_dong hd 
left join hop_dong_chi_tiet hdct on hdct.ma_hop_dong = hd.ma_hop_dong
group by hd.ma_hop_dong;

-- 11 Hiển thị thông tin các dịch vụ đi kèm đã được sử dụng bởi những khách hàng có ten_loai_khach là “Diamond” và có dia_chi ở “Vinh” hoặc “Quảng Ngãi”.
select dvdk.* , kh.ma_khach_hang, kh.ho_ten ,kh.dia_chi from khach_hang kh 
...
select hd.ma_hop_dong , nv.ho_ten,kh.ho_ten , kh.so_dien_thoai , dv.ten_dich_vu, coalesce(sum(hdct.so_luong),0) as so_luong_dich_vu_di_kem , hd.tien_dat_coc ,hd.ngay_lam_hop_dong
from hop_dong hd 
...
join nhan_vien nv on nv.ma_nhan_vien = hd.ma_nhan_vien
join dich_vu dv on dv.ma_dich_vu = hd.ma_dich_vu 
and year(hd.ngay_lam_hop_dong) = 2021 and month(hd.ngay_lam_hop_dong) in (1,2,3,4,5,6)
group by hd.ma_hop_dong
) 
group by hd.ma_hop_dong;
...
select dvdk.ten_dich_vu_di_kem , sum(hdct.so_luong) as  so_luong_su_dung from dich_vu_di_kem dvdk
join  hop_dong_chi_tiet hdct on hdct.ma_dich_vu_di_kem = dvdk.ma_dich_vu_di_kem
group by dvdk.ten_dich_vu_di_kem
having  so_luong_su_dung = (
select sum(hdct.so_luong) as  so_luong_su_dung from dich_vu_di_kem dvdk
join  hop_dong_chi_tiet hdct on hdct.ma_dich_vu_di_kem = dvdk.ma_dich_vu_di_kem
group by dvdk.ten_dich_vu_di_kem
...
-- Thông tin hiển thị bao gồm ma_hop_dong, ten_loai_dich_vu, ten_dich_vu_di_kem, so_lan_su_dung (được tính dựa trên việc count các ma_dich_vu_di_kem).
 ...
 select  hd.ma_hop_dong,ldv.ten_loai_dich_vu , bang_tam.ten_dich_vu_di_kem , bang_tam.so_lan_su_dung from 
 ( select dvdk.ten_dich_vu_di_kem as ten_dich_vu_di_kem , dvdk.ma_dich_vu_di_kem as ma_dich_vu_di_kem ,count(hdct.ma_dich_vu_di_kem) as so_lan_su_dung  from hop_dong_chi_tiet hdct 
...
left join hop_dong hd on hd.ma_nhan_vien = nv.ma_nhan_vien and year(hd.ngay_lam_hop_dong) in (2019,2021)
...
join loai_dich_vu ldv on ldv.ma_loai_dich_vu = dv.ma_loai_dich_vu and year(hd.ngay_lam_hop_dong) = '2021'
