...
<div>
    <form action="products?action=add" method="post">
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="nameProductInput" name="name_product" required>
        </div>
        <div class="mb-3">
            <label for="priceInput" class="form-label">Giá</label>
            <input type="number" class="form-control" id="priceInput" name="price" step="0.01" min="0" required>
        ...
        <div class="mb-3">
            <select class="form-select form-select-sm" aria-label=".form-select-sm example" name="id_category">
                <c:forEach items="${category}" var="c" >
                    <option selected value="${c.getId_category()}">${c.getName_category()}</option>
                </c:forEach>
            </select>
        </div>
